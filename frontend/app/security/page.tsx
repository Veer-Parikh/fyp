...
export default function SecurityPage() {
  ...
  // Kill-chain specific state
  const [killchain, setKillchain] = useState<KillchainState>(null)
  const [killchainLoading, setKillchainLoading] = useState(false)
  const [killchainError, setKillchainError] = useState<string | null>(null)
  const [graphPng, setGraphPng] = useState<string | null>(null)


  // Prevent double-clicking
  const isScanning = useRef(false)
  const prettyRelation = (raw: string | undefined): string => {
  if (!raw) return ""
  ...
  const handleGenerateKillchain = async () => {
    if (!results?.result) {
      setKillchainError("Run a scan first.");
      return;
    }
    ...
    try {
      ...
      setKillchainError(err.message);
    } finally {
      setKillchainLoading(false);
    }
  };


  const handleCaptureForPdfClick = async () => {
    if (!killchain) {
      alert("Generate the kill-chain graph first, then adjust nodes and capture.")
      return
    }

    const png = await captureGraphPNG()
    if (png) {
      alert('Graph captured for PDF.\nNow click "Generate Kill-Chain Mapping" again to refresh the PDF with this layout.')
    } else {
      alert("Could not capture graph. Make sure the graph is visible on screen.")
    }
  }

  const handleDownloadKillchainPdf = async () => {
    if (!killchain) {
      setKillchainError("Generate kill-chain mapping first.");
      return;
    }

    // 1. Capture graph screenshot
    const graph_png = await captureGraphPNG();
    if (!graph_png) {
      setKillchainError("Failed to capture graph.");
      return;
    }
    console.log("Killchain payload:", {
      result: {
        ...results.result,
        graph_png
      }
    ...
    const response = await fetch("http://localhost:8000/security/killchain", {
      ...
      body: JSON.stringify({
        result: {
          ...results.result,
          graph_png, // send PNG properly
        }
      }),
    });

    if (!response.ok) {
      setKillchainError("PDF generation failed.");
      return;
    }

    // 3. Receive PDF and auto-download
    const data = await response.json();

    const pdfBlob = await fetch(`data:application/pdf;base64,${data.pdf_base64}`)
      .then(r => r.blob());

    const url = URL.createObjectURL(pdfBlob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "killchain-report.pdf";
    a.click();
  };
            ...
            <div className="space-y-10 mt-10">
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  ...
                  disabled={killchainLoading}
                  className="px-4 py-2 rounded-lg border border-accent/60 bg-accent/10 hover:bg-accent/30 text-accent font-mono text-sm"
                >
                  {killchainLoading ? "Building Kill-Chain..." : "Generate Kill-Chain Mapping"}
                </button>
              </div>

              {/* Kill-Chain Mapping Section ALWAYS at top of results */}
              <div className="space-y-4 mb-6">
                <div className="flex items-center justify-between gap-4 mt-2">
                  <h3 className="text-3xl font-mono font-bold cyber-gradient-text">
                    Kill-Chain Mapping
                  </h3>
                  <div className="flex items-center gap-3">
                    {killchain && (
                      <button
                        onClick={handleCaptureForPdfClick}
                        className="px-3 py-2 rounded-lg border border-accent/60 bg-accent/10 hover:bg-accent/30 text-accent font-mono text-xs"
                      >
                        Capture Graph for PDF
                      </button>
                    )}
                    {killchain?.pdf_base64 && (
                      <button
                        onClick={handleDownloadKillchainPdf}
                        className="px-4 py-2 rounded-lg border border-purple-400/70 
                                   bg-purple-500/10 hover:bg-purple-500/30 
                                   text-purple-300 font-mono text-xs transition-all"
                      >
                ...
                <Card className="bg-[#020617] border border-accent/40 shadow-[0_0_25px_rgba(0,255,200,0.15)] rounded-xl">
                  <CardHeader>
                    <CardTitle className="font-mono text-accent text-sm">
                      ⚡ Attack Graph (Interactive)
                    </CardTitle>
                    <CardDescription className="text-xs text-muted-foreground">
                      Drag nodes, zoom, explore exploitation routes, and analyze attack propagation visually.
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="h-[520px] rounded-xl overflow-hidden border border-accent/30 shadow-inner relative">
                      {killchainLoading && (
                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-black/40">
                          <Loader />
                          <p className="mt-2 text-xs font-mono text-muted-foreground">
                            Mapping attack surface, building graph, and extracting kill-chain paths...
                          </p>
                        </div>
                      )}

                      {!killchain && !killchainLoading && (
                        <div className="flex items-center justify-center h-full text-sm text-muted-foreground font-mono">
                          Run a scan and click &quot;Generate Kill-Chain Mapping&quot; to visualize the attack graph.
                        </div>
                      )}
...
>
  <ReactFlow
    nodes={killchain.graph.nodes.map((n: any) => {
      const basePos = n.position || { x: 0, y: 0 }

      // 🔥 stretch the graph horizontally so relation labels have room
      const STRETCH_X = 1.8
      const pos = {
        x: basePos.x * STRETCH_X,
        y: basePos.y,
      }
        ...
        style: {
          ...
          background:
            n.type === "threat"
              ...
              : n.type === "vuln"
              ? "linear-gradient(135deg,#0f172a,#854d0e)"
              : n.type === "port"
              ? "linear-gradient(135deg,#022c22,#14532d)"
              : "linear-gradient(135deg,#020617,#075985)",
          color: "#e5e7eb",
          fontSize: 12,
          boxShadow:
            "0 0 18px rgba(0,255,180,0.2), inset 0 0 12px rgba(0,255,150,0.15)",
          transition: "transform 0.2s ease",
        },
      }
    })}
    edges={killchain.graph.edges.map((e: any, i: number) => ({
      id: `edge-${i}`,
      source: e.source,
      target: e.target,
      type: "smoothstep",
      animated: true,
      label: prettyRelation(e.label),
      markerEnd: {
      ...
      // make the relationship text pop + readable
      labelStyle: {
        fontSize: 14,
        fontWeight: 700,
        fill: "#ffffff",
      ...
      labelShowBg: true,
      labelPosition: 0.5, // center of edge
    }))}
    fitView
    fitViewOptions={{ padding: 0.3 }}
  >
    <Background gap={18} />
    {/* controls restored – zoom in/out + fit view */}
    <Controls
  style={{
    background: "#ffffffcc",
    borderRadius: 6,
...
/>
                ...
                {killchainError && (
                  <Card className="bg-red-500/10 border border-red-500/40">
                    <CardContent className="py-4">
                      <p className="font-mono text-sm text-red-400">{killchainError}</p>
                    </CardContent>
                  </Card>
                )}
                ...
                {/* Attack Paths */}
                {killchain && !killchainLoading && (
                  <Card className="border-orange-400/40 bg-card/30">
                    <CardHeader>
                      <CardTitle className="font-mono">Attack Path Analysis</CardTitle>
                      <CardDescription>
                        Correlated vulnerabilities and services forming potential kill-chains
                      </CardDescription>
                    </CardHeader>

                    <CardContent className="space-y-4 font-mono text-sm">
                      {killchain.attack_paths.length === 0 ? (
                        <p className="text-muted-foreground">
                          No attack paths detected.
                        </p>
                      ) : (
                        killchain.attack_paths.map((path: any, i: number) => (
                          <div key={path.id || i} className="border border-border rounded p-3 space-y-1">
                            <div className="flex items-center justify-between">
                              <span className="font-semibold">{path.threat}</span>
